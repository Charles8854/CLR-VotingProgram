    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::AgentProfile(agent_profile) => validate_create_agent_profile(
                    EntryCreationAction::Create(action),
                    agent_profile,
                ),
                EntryTypes::Ballot(ballot) => {
                    validate_create_ballot(EntryCreationAction::Create(action), ballot)
                }
                EntryTypes::VoteItem(vote_item) => {
                    validate_create_vote_item(EntryCreationAction::Create(action), vote_item)
                }
            },
            OpEntry::UpdateEntry {
                app_entry, action, ..
                ...
                EntryTypes::Ballot(ballot) => {
                    validate_create_ballot(EntryCreationAction::Update(action), ballot)
                }
                EntryTypes::VoteItem(vote_item) => {
                    validate_create_vote_item(EntryCreationAction::Update(action), vote_item)
                }
            },
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::VoteItem(vote_item) => {
                        ...
                        let original_vote_item = match VoteItem::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get VoteItem from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_vote_item(
                            action,
                            vote_item,
                    ...
                    EntryTypes::Ballot(ballot) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_ballot = match Ballot::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Ballot from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_ballot(
                            action,
                            ballot,
                            original_create_action,
                            original_ballot,
                    ...
                    EntryTypes::AgentProfile(agent_profile) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_agent_profile =
                            match AgentProfile::try_from(original_app_entry) {
                                Ok(entry) => entry,
                                Err(e) => {
                                    return Ok(ValidateCallbackResult::Invalid(format!(
                                        "Expected to get AgentProfile from Record: {e:?}"
                                    )));
                                }
                            };
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::VoteItem(original_vote_item) => validate_delete_vote_item(
                    delete_entry.clone().action,
                    original_action,
                    original_vote_item,
                ),
                EntryTypes::Ballot(original_ballot) => validate_delete_ballot(
                    delete_entry.clone().action,
                    original_action,
                    original_ballot,
                ),
                EntryTypes::AgentProfile(original_agent_profile) => validate_delete_agent_profile(
                    delete_entry.clone().action,
                    original_action,
                    original_agent_profile,
                ),
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::AgentProfileUpdates => validate_create_link_agent_profile_updates(
                action,
                base_address,
                target_address,
                tag,
            ),
            LinkTypes::BallotUpdates => {
                validate_create_link_ballot_updates(action, base_address, target_address, tag)
            }
            LinkTypes::VoteItemUpdates => {
                validate_create_link_vote_item_updates(action, base_address, target_address, tag)
            }
        },
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::AgentProfileUpdates => validate_delete_link_agent_profile_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::BallotUpdates => validate_delete_link_ballot_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::VoteItemUpdates => validate_delete_link_vote_item_updates(
                action,
                original_action,
                base_address,
                target_address,
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::AgentProfile(agent_profile) => {
                            let result = validate_create_agent_profile(
                                EntryCreationAction::Update(action.clone()),
                                agent_profile.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_agent_profile: Option<AgentProfile> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_agent_profile = match original_agent_profile {
                                    Some(agent_profile) => agent_profile,
                                    None => {
                                        return Ok(
                        ...
                        EntryTypes::Ballot(ballot) => {
                            let result = validate_create_ballot(
                                EntryCreationAction::Update(action.clone()),
                                ballot.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_ballot: Option<Ballot> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_ballot = match original_ballot {
                                    Some(ballot) => ballot,
                                    None => {
                                        return Ok(
                                            ValidateCallbackResult::Invalid(
                        ...
                        EntryTypes::VoteItem(vote_item) => {
                            let result = validate_create_vote_item(
                                EntryCreationAction::Update(action.clone()),
                                vote_item.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_vote_item: Option<VoteItem> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_vote_item = match original_vote_item {
                                    Some(vote_item) => vote_item,
                                    None => {
                                        return Ok(
                                            ValidateCallbackResult::Invalid(
                                                "The updated entry type must be the same as the original entry type"
                ...
                OpRecord::DeleteEntry {
                    ...
                    match original_app_entry {
                        EntryTypes::AgentProfile(original_agent_profile) => {
                            validate_delete_agent_profile(
                                action,
                                original_action,
                                original_agent_profile,
                            )
                        }
                        EntryTypes::Ballot(original_ballot) => {
                            validate_delete_ballot(action, original_action, original_ballot)
                        }
                        EntryTypes::VoteItem(original_vote_item) => {
                            validate_delete_vote_item(action, original_action, original_vote_item)
                        }
                    }
                }
                ...
                OpRecord::CreateLink {
                    ...
                    LinkTypes::BallotUpdates => validate_create_link_ballot_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
                    ),
                    LinkTypes::VoteItemUpdates => validate_create_link_vote_item_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
                ...
                OpRecord::DeleteLink {
                    ...
                    match link_type {
                        LinkTypes::AgentProfileUpdates => {
                            validate_delete_link_agent_profile_updates(
                                action,
                                create_link.clone(),
                                base_address,
